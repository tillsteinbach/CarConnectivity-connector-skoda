...
The connector retries automatically. On MQTT authentication failures it refreshes the Android FCM registration, uploads the token to Skoda's notifications API again, refreshes the MQTT connect token, and then waits before retrying. FCM refreshes are throttled to avoid repeatedly registering new notification tokens while Skoda's backend is still propagating the previous one. Recovery can take a few minutes. If MQTT still does not connect after repeated retries, check the logs for notification registration failures before resetting credentials manually.
