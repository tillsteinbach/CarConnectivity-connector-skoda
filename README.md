...
The connector retries automatically and refreshes the token again before the next MQTT connection attempt. If Skoda keeps rejecting the token, the connector performs one controlled notification re-registration and then waits before retrying. Recovery can take a few minutes. If MQTT still does not connect after the retry limit is reached, check the logs for notification registration failures before resetting credentials manually.
